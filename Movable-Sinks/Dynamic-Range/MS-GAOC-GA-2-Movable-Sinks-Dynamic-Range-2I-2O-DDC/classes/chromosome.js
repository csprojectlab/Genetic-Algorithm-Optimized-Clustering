class Chromosome {
    ...
    static validGene (gene, index) {
        ...
        heads.forEach (h_index => {         // For each head index check for collision with index
            let vicinity2 = network.nodes[h_index].vicinity;
            if (network.nodeDistance[h_index][index] <= vicinity1 || network.nodes[h_index][index] <= vicinity2)
                return false;
        });
        return true;
    }
            ...
            if (network.nodes[index].resEnergy > 0) {
                if (value) {
                    network.nodeDistance[index].forEach ((d, i) => {
                        if (!usedNodes.includes (i) && d < network.nodes[index].vicinity) {
                            count++;
                            usedNodes.push (i);
                        }
                    });
    ...
    crossover (other_parent) {
        ...
        for (let i = 0; i < child.length; i++) {
            if (child[i] == 1) {
                if (!Chromosome.validGene (child, i))
                    return null;
            }
        }
