class Chromosome {
    ...
    isValid (gene, index) {
        ...
        for (let i = 0; i < gene.length; i++) {
            if (i != index && gene[i] == 1) {
                if (network.nodeDistance[i][index] < network.nodes[index].vicinity)     // DYNAMIC NEECP
                    return false;
            }
        }
        return true;
            ...
            if (network.nodes[index].resEnergy > 0) {
                if (value) {
                    network.nodeDistance[index].forEach ((d, i) => {
                        if (!usedNodes.includes (i) && d < network.nodes[index].vicinity) {     // DYNAMIC NEECP
                            count++;
                            usedNodes.push (i);
                        }
                    });
    ...
    crossover (other_parent) {      // DYNAMIC update
        let child = [];
