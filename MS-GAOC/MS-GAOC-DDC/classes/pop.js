class Population {
    ...
    generateClusters () {
        ...
        network.nodes.forEach ((node, index) => {
            if (!heads.includes (index) && network.nodes[index].resEnergy > 0) {
                let headIndex = -1;
                let sIndex = Utils.closestSink (network.sinkDistance, index, network.sinks);                
                if (network.sinkDistance[index][sIndex] > NON_CLUSTER_RANGE)
                    headIndex = Utils.closestHead (network.nodeDistance, index, heads);
    ...
    display () {
        // network.display (this.chromosomes[this.fittestIndex].genes, obj.C, obj.NCN)
        network.sinks.forEach (sink => sink.display ())
        // Display the best network cluster
